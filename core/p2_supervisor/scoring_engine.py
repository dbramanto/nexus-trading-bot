...
class ScoringEngine:
    ...
    def score(self, p1_reports, circuit_breaker_state=None):
        ...
        # Log P2 result
        logger.info(
            f"P2 SCORE | Score={final:.1f} Grade={grade} Bias={bias} Regime={regime} | "
            f"T0={t0:.1f} T1={t1:.1f} T2={t2:.1f} Pen={pen_total:.1f} Bon={bon_total:.1f}"
        )
    ...
    def _determine_bias(self, modules):
        ...
        logger.info("VOTE MSS: CHoCH=%s BOS=%s | BULLISH=%d BEARISH=%d" % (choch_dir, bos_dir, votes["BULLISH"], votes["BEARISH"]))
        ...
        logger.info("VOTE MOMENTUM: dir=%s str=%d rej=%s | BULLISH=%d BEARISH=%d" % (mom_dir, mom_str, rejection, votes["BULLISH"], votes["BEARISH"]))
        ...
        # Threshold: >= 4 votes untuk declare bias
        if votes["BULLISH"] >= 2: return "BULLISH"  # OPTION B: 4→2
        if votes["BEARISH"] >= 2: return "BEARISH"  # OPTION B: 4→2
        logger.info("BIAS VOTING [%s] | BULLISH=%s BEARISH=%s → NEUTRAL" % (modules.get("symbol", "UNK"), votes["BULLISH"], votes["BEARISH"]))
        return "NEUTRAL"
    ...
    def _build_p1_snapshot(self, modules):
        ...
        for name, rep in modules.items():
            # Skip non-dict values (like symbol string)
