...
class StrategyLogic:
    ...
    def evaluate(self, context_package: Dict[str, Any], circuit_breaker_active: bool = False) -> Dict[str, Any]:
        ...
        # === PATTERN FILTER (Config-Driven) ===
        if self.config.rules.PATTERN_FILTER_ENABLED:
            p1_snap = context_package.get("p1_snapshot", {})
            pattern = p1_snap.get("candle_pattern", {}).get("primary_pattern")
            
            if pattern not in self.config.rules.APPROVED_PATTERNS:
                return self._wait(
                    f"Pattern {pattern} not in approved list {self.config.rules.APPROVED_PATTERNS}",
...
#             return self._wait(f"Regime RANGING — skip", score, grade, threshold)
        ...
        if bias == "BULLISH": 
            ...
            # SHORT DISABLED for data collection
            return self._wait("SHORT disabled - LONG only", score, grade, threshold)
        else: 
    ...
    def _wait(self, reason, score, grade, threshold):
        # Log rejection
        logger.info(f"P3 REJECT | Reason: {reason} | Score={score:.1f} Grade={grade}")
        return {
            "action": "NO_TRADE",
            "action": "WAIT",
            "grade": grade,
