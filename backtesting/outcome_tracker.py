...
def process_shadow_log(log_file, client, output_csv):
    ...
    for entry in to_label:
        ...
        row = {
            ...
            "oi_signal": snap.get("open_interest", {}).get("oi_signal"),
            "oi_change_pct": snap.get("open_interest", {}).get("oi_change_pct"),
            "consolidating": snap.get("consolidation_detector", {}).get("consolidating"),
            "consolidation_quality": snap.get("consolidation_detector", {}).get("quality"),
            "spike_detected": snap.get("spike_classifier", {}).get("spike_detected"),
            "spike_direction": snap.get("spike_classifier", {}).get("spike_direction"),
            "ha_direction": snap.get("heiken_ashi", {}).get("ha_direction"),
            "ha_strength": snap.get("heiken_ashi", {}).get("ha_strength"),
            "ha_consistent": snap.get("heiken_ashi", {}).get("consistent"),
            "hypothesis_signal": entry.get("hypothesis_signal"),
            "hypothesis_bull_score": entry.get("hypothesis_bull_score"),
            "hypothesis_bear_score": entry.get("hypothesis_bear_score"),
            "model_b_confidence": entry.get("model_b_confidence"),
            "model_b_action": entry.get("model_b_action"),
            "model_c_action": entry.get("model_c_action"),
            "session": entry.get("session"),
            "btc_trend": entry.get("btc_trend"),
            "btc_price_change_pct": entry.get("btc_price_change_pct"),
            "rs_vs_btc": entry.get("rs_vs_btc"),
        }
        labeled.append(row)
        processed += 1
        time.sleep(0.2)
    ...
    if labeled:
        ...
        if os.path.exists(output_csv):
            ...
            # Safe concatenation to avoid FutureWarning
            if not df_existing.empty:
                df_out = pd.concat([df_existing, df_out], ignore_index=True)
            # else: df_out already contains new data
        df_out.to_csv(output_csv, index=False)
        logger.info(f"Saved {len(labeled)} labeled trades to {output_csv}")

    # Update shadow log to prevent re-processing
    if labeled:
        labeled_ts_symbols = {(row['timestamp'], row['symbol']) for row in labeled}
        ...
        for entry in entries:
            ...
            # Mark as labeled if processed
            if (ts, sym) in labeled_ts_symbols:
                # Find the outcome from labeled data
                matching = [r for r in labeled if r['timestamp'] == ts and r['symbol'] == sym]
                if matching:
                    entry['outcome'] = matching[0]['outcome']
            
            updated_entries.append(entry)
        
        # Write back to shadow log
        with open(log_file, 'w') as f:
            for entry in updated_entries:
                f.write(json.dumps(entry) + '\n')
        
        logger.info(f"Updated {len(labeled)} entries in shadow log to prevent re-processing")
