...
*Win Rate:* {yesterday.get('win_rate', 0):.1f}%
...
Status: {'✅ Profitable' if report.get('pnl', 0) > 0 else '⚠️ Break-even' if report.get('pnl', 0) == 0 else '❌ Loss'}"""
...
🏆 BEST DAY
{report.get('best_day', {}).get('date', 'N/A')}: ${report.get('best_day', {}).get('pnl', 0):+,.2f}

📉 WORST DAY
{report.get('worst_day', {}).get('date', 'N/A')}: ${report.get('worst_day', {}).get('pnl', 0):+,.2f}

Days active: {report.get('days_count', 0)}
Status: {'✅ Profitable week' if report.get('total_pnl', 0) > 0 else '⚠️ Break-even' if report.get('total_pnl', 0) == 0 else '❌ Loss'}"""
        
        return self.send_message(message)
    
    def notify_monthly_report(self, report: Dict) -> bool:
        """
        Send monthly report notification
        
        Args:
            report: Monthly report data
...
🏆 BEST WEEK
{report.get('best_week', {}).get('period', 'N/A')}
P&L: ${report.get('best_week', {}).get('pnl', 0):+,.2f}

📉 WORST WEEK
{report.get('worst_week', {}).get('period', 'N/A')}
P&L: ${report.get('worst_week', {}).get('pnl', 0):+,.2f}

Weeks: {report.get('weeks_count', 0)}
Status: {'🏆 Profitable month!' if report.get('total_pnl', 0) > 0 else '⚠️ Break-even' if report.get('total_pnl', 0) == 0 else '📉 Loss - needs optimization'}"""
